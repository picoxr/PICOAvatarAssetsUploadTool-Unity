...
namespace ICSharpCode.SharpZipLib.Zip
{
	...
	public static class ZipStrings
	{
		static ZipStrings()
		{
			try
			{
				...
				SystemDefaultCodePage =
					(platformCodepage == 1 || platformCodepage == 2 || platformCodepage == 3 || platformCodepage == 42)
						? FallbackCodePage
						: platformCodepage;
			}
			catch
		...
		{
			get { return codePage == AutomaticCodePage ? Encoding.UTF8.CodePage : codePage; }
			set
			{
		...
		{
			get { return codePage == Encoding.UTF8.CodePage; }
			set
			{
				if (value)
				{
		...
		private static Encoding EncodingFromFlag(int flags)
			=> ((flags & (int)GeneralBitFlags.UnicodeText) != 0)
				...
				: Encoding.GetEncoding(
					...
					codePage == AutomaticCodePage ? SystemDefaultCodePage : codePage);
