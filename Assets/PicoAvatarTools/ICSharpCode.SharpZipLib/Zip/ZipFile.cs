			...
			lock (baseStream_)
			{
				...
				if (signature != ZipConstants.LocalHeaderSignature)
				{
					throw new ZipException(string.Format(
						"Wrong local header signature at 0x{0:x}, expected 0x{1:x8}, actual 0x{2:x8}",
				...
				else
				{
					...
					if (entries_.Length == 0)
					{
						byte[] theComment = (newComment_ != null)
							...
							: ZipStrings.ConvertToArray(comment_);
		...
		*/
		...
		private void RunUpdates()
		{
			...
			try
			{
				...
				byte[] theComment =
					(newComment_ != null) ? newComment_.RawComment : ZipStrings.ConvertToArray(comment_);
